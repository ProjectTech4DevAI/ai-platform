...
from app.models import DocTransformationJobPublic, DocTransformationJobsPublic, TransformedDocumentPublic
...
def get_transformation_job(
    ...
    include_url: bool = Query(False, description="Include a signed URL for the transformed document"),
):
    ...
    if getattr(job, "transformed_document_id", None):
        ...
        if include_url:
                ...
                transformed_doc_schema.signed_url= storage.get_signed_url(document.object_store_url)

    job_schema = DocTransformationJobPublic.model_validate(job, from_attributes=True)
    job_schema = job_schema.model_copy(update={"transformed_document": transformed_doc_schema})
...
def get_multiple_transformation_jobs(
    ...
    include_url: bool = Query(False, description="Include a signed URL for each transformed document"),
):
