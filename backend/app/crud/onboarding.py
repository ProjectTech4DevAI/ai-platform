...
def onboard_project(session: Session, onboard_in: OnboardingRequest) -> OnboardingResponse:
    ...
    project_create = ProjectCreate(name=onboard_in.project_name, organization_id=organization.id)
    ...
    if not user:
        user_create = UserCreate(email=onboard_in.email, full_name=onboard_in.user_name, password=onboard_in.password)
    ...
    if onboard_in.openai_api_key:
        creds = {
            "api_key": onboard_in.openai_api_key
        }
