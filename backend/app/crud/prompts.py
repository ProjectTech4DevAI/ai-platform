...
def create_prompt(
    session: Session, prompt_in: PromptCreate, project_id: int
) -> tuple[Prompt, PromptVersion]:
    """
    Create a new prompt and its first version.
    """
    prompt = Prompt(
        name=prompt_in.name, description=prompt_in.description, project_id=project_id
    )
    session.add(prompt)
    session.flush()
...
def get_prompts(
    ...
    stmt = (
        select(Prompt)
        .where(Prompt.project_id == project_id, Prompt.is_deleted.is_(False))
...
def get_prompt_by_id(
    session: Session, prompt_id: UUID, project_id: int, include_versions: bool = False
    ...
    if include_versions:
        join_condition = and_(
            PromptVersion.prompt_id == Prompt.id, PromptVersion.is_deleted.is_(False)
        )
    ...
    if not results:
        logger.error(
            f"[get_prompt_by_id] Prompt not found | ID: {prompt_id}, Project ID: {project_id}"
        )
        raise HTTPException(status_code=404, detail="Prompt not found")
...
def update_prompt(
    ...
    prompt = prompt_exists(session=session, prompt_id=prompt_id, project_id=project_id)
