...
def test_create_prompt_version_route_success(
    ...
    prompt, _ = create_prompt(db, prompt_in=prompt_in, project_id=user_api_key.project_id)

    version_in = PromptVersionCreate(
        instruction="Version 2 instructions",
    ...
    assert data["version"] == 2  # First version created by create_prompt, this is second
