...
def mock_presidio():
    """
    Provide patched Presidio AnalyzerEngine and AnonymizerEngine for tests and configure their default behaviors.
    
    Patches the AnalyzerEngine and AnonymizerEngine used by the PIIRemover so tests can run without the real Presidio services. The patched AnalyzerEngine's `analyze` returns an empty list. The patched AnonymizerEngine's `anonymize` returns a mock object with `text = "redacted text"`.
    
    Returns:
        (mock_analyzer, mock_anonymizer): A tuple containing the patched AnalyzerEngine and AnonymizerEngine mock objects.
    """
...
def validator(mock_presidio):
    """
    Create a PIIRemover validator configured for testing.
    
    Parameters:
        mock_presidio: A pytest fixture that patches Presidio's AnalyzerEngine and AnonymizerEngine to provide controlled analyzer/anonymizer behavior.
    
    Returns:
        PIIRemover: An instance configured with entity_types=None, threshold=0.5, language="en", and a mocked language_detector whose `predict` and `is_hindi` behavior can be controlled in tests.
    """
    return PIIRemover(
        entity_types=None,
        threshold=0.5,
