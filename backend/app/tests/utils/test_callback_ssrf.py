...
class TestSendCallback:
    ...
    @patch("app.utils.validate_callback_url")
    @patch("requests.Session")
    def test_callback_response_size_limit_exceeded(
        self, mock_session_class, mock_validate
    ):
        """Test that callback rejects responses exceeding size limit."""
        mock_session = MagicMock()
        mock_response = MagicMock()
        mock_response.raise_for_status.return_value = None
        ...
        mock_response.iter_content.return_value = [
            b"x" * chunk_size for _ in range(num_chunks)
        ]
    ...
    @patch("app.utils.validate_callback_url")
    @patch("requests.Session")
    def test_callback_response_size_within_limit(
        self, mock_session_class, mock_validate
    ):
        """Test that callback accepts responses within size limit."""
        mock_session = MagicMock()
        mock_response = MagicMock()
        mock_response.raise_for_status.return_value = None
    ...
    @patch("app.utils.validate_callback_url")
    @patch("requests.Session")
    def test_callback_uses_streaming(self, mock_session_class, mock_validate):
        """Test that callback uses streaming to prevent loading entire response into memory."""
        mock_session = MagicMock()
        mock_response = MagicMock()
        mock_response.raise_for_status.return_value = None
        mock_response.iter_content.return_value = [b"test"]
