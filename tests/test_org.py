...
def test_post_org(client: TestClient) -> None:
...
def test_get_org(db: Session, client: TestClient) -> None:
...
def test_get_multiple_orgs(db: Session, client: TestClient) -> None:
    for _ in range(5):
...
def test_update_org(db: Session, client: TestClient) -> None:
    user = generators.create_user(db)
    new_name = fake.name()
...
def test_delete_org(db: Session, client: TestClient, mocker: MockerFixture) -> None:
    user = generators.create_user(db)
...
def test_delete_db_org(db: Session, mocker: MockerFixture, client: TestClient) -> None:
    user = generators.create_user(db)
